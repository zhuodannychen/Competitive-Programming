...
ll solve(int idx, int tight, int prev, bool seen, string& n) {
    if (idx >= n.size())  {
        return 1;
    }

    if (dp[idx][tight][prev][seen] != -1)
        return dp[idx][tight][prev][seen];

    ll total = 0;
    int range = (tight ? n[idx]-'0' : 9);
    for (int i = 0; i <= range; i++) {
        int newtight = (i == n[idx]-'0' ? tight : 0);
        if (i == 0 && !seen)
            total += solve(idx+1, newtight, i, false, n) % MOD;
        else if (i != prev)
            total += solve(idx+1, newtight, i, true, n) % MOD;
    }
    return dp[idx][tight][prev][seen] = total;
}
...
int main() {
    ...
    MEM(dp, -1);
    ll upper = solve(0, 1, -1, false, r);
    MEM(dp, -1);
    ll lower = solve(0, 1, -1, false, l);
    cout << (upper - lower + MOD) % MOD << endl;
}
