...
ll minimax(vll options, ll n, ll m) {
    if (m > n)
        return 0;
    if (m == n)
        return -1;
    ...
    ll best = 1;
    for (auto prime : options)
        best = min(best, minimax(options, n, prime * m));
    dp[m] = -best;
    return -best;
}
...
int main() {
    ...
    while (t--) {
        ...
        int res = minimax(divisors(n), n, 1);
        if (res == -1)
            cout << (start == "Alice" ? "Bob" : "Alice") << endl;
        else if (res == 0)
            cout << "tie\n";
        else
            cout << start << endl;
