...
int main() {
    ...
    while (t--) {
        ...
        f0r (j, 8) {
            int rank1 = pref[n-1][offbyone[j][0]];
            int rank2 = pref[n-1][offbyone[j][1]];
            int rank3 = pref[n-1][offbyone[j][2]];

            if (rank1 < rank2 && rank1 < rank3) {
                dp[n-1][j] = offbyone[j][0];
            } else if (rank2 < rank3) {
                dp[n-1][j] = offbyone[j][1];
            } else {
                dp[n-1][j] = offbyone[j][2];
            }
        }

        for (int i = n-2; i >= 0; i--) {
            f0r (j, 8) {
                int rank1 = pref[i][dp[i+1][offbyone[j][0]]];
                int rank2 = pref[i][dp[i+1][offbyone[j][1]]];
                int rank3 = pref[i][dp[i+1][offbyone[j][2]]];

                if (rank1 < rank2 && rank1 < rank3) {
                    dp[i][j] = dp[i+1][offbyone[j][0]];
                } else if (rank2 < rank3) {
                    dp[i][j] = dp[i+1][offbyone[j][1]];
                } else {
                    dp[i][j] = dp[i+1][offbyone[j][2]];
                }
            }
        }
        print(dp[0][0]);
