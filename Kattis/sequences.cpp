...
int main() {
    ...
    f0r (i, s.size()) {
        if (s[i] == '0') {
            // for each 1 to its left, there's an inversion. There are 2**questions sequences we need
            // to consider at this point

            ans = (ans + ones * pow2[questions]) % MOD;
            // We still need to consider the combinations of the question marks.
            // There are n question marks, the total number of combinations is 2^n. Since we considered
            // all the combinations, exactly half should be 1. Therefore, 2^n / 2 = 2^(n-1).

            if (questions > 0)
                ans = (ans + questions * pow2[questions-1]) % MOD;
        } else if (s[i] == '1') {
            ones++;
        } else {
            // Since each ? produces two new combinations, we multiply the original answer by 2
            ans = 2 * ans % MOD;
            // Assume ? was a 0, then we treat it as a zero 
            ans = (ans + ones * pow2[questions]) % MOD;
            if (questions > 0)
                ans = (ans + questions * pow2[questions-1]) % MOD;
            // ? = 1 doesn't affect current answer and is accounted in the 0's section
            questions++;
        }
    }
    cout << ans % MOD << endl;
