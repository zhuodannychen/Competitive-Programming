...
bool dfs(int pos, string s, int i, int j) {
    if (pos == s.size())
        return true;
    if (i < 0 || i >= 4 || j < 0 || j >= 4 || vis[i][j] || s[pos] != grid[i][j])
        return false;
    ...
    while (boards--) {
        ...
        for (string word : words) {
            if (valid(word)) {
                ...
                if (sz > longest.size())
                    longest = word;
                else if (sz == longest.size() && word < longest)
                    longest = word;
                score += scores[sz];
                total++;
            }
        }
        cout << score << " " << longest << " " << total << endl;
    }
}
