...
public:
    UnionFind(int N) {
        setSize.assign(2*N+5, 1);
        setSum.assign(2*N+5, 0);
        numSets = N; rank.assign(2*N+5, 0);
        p.assign(2*N+5, 0);
        for (int i = 1, j=N+1; i <= N; i++, j++) {
            // keep two copies to distinguish move and union
            p[i] = j;
            p[j] = j;
            setSize[j] = 1;
            setSum[j] = i;
    ...
    ll sumOfSet(int i) { return setSum[findSet(i)]; }
};
