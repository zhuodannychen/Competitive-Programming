    ...
    while (true) {
        ...
        while (!q.empty()) {
            ...
            for (int dr = 0; dr < 6; dr++) {
                int newx = dx[dr] + cur.x;
                int newy = dy[dr] + cur.y;
                int newz = dz[dr] + cur.z;
                if (newx < 0 || newx >= l || newy < 0 || newy >= r || newz < 0 || newz >= c || grid[newx][newy][newz] == '#' || dis[newx][newy][newz] != -1)
                    continue;
                dis[newx][newy][newz] = dis[cur.x][cur.y][cur.z] + 1;
