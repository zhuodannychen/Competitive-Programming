...
int main() {
    ...
    int t; cin >> t;
    while (t--) {
        int towns, start, total, time; cin >> towns >> start >> total >> time;
        time++;
        int SINK = towns * time + 1;
        max_flow<110000> network;
        network.add(0, start, total);

        // connect med at each timestamp to sink
        int m; cin >> m;
        for (int i = 0; i < m; i++) {
            int a; cin >> a;
            for (int j = 0; j < time; j++) {
                int loc = j * towns + a;
                network.add(loc, SINK, inf);
            }
        }

        // connect each town to itself because we can wait at a town
        for (int i = 1; i < time; i++) {
            for (int j = 1; j <= towns; j++) {
                int loc1 = (i-1) * towns + j;
                int loc2 = i * towns + j;
                network.add(loc1, loc2, inf);
            }
        }

        int r; cin >> r;
        // connect each road to town
        for (int i = 0; i < r; i++) {
            int a, b, cap, dis; cin >> a >> b >> cap >> dis;
            for (int j = 0; j < time; j++) {
                if (j + dis >= time)
                    break;
                int loc1 = j * towns + a;
                int loc2 = (j+dis) * towns + b;
                network.add(loc1, loc2, cap);
            }
        }

        cout << network.calc(0, SINK) << endl;
    }
}
