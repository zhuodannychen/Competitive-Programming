...
void markOcean(int r, int c, int R, int C) {
    if (val_bounds(r, c, R, C) && grid[r][c] == '0') {
        grid[r][c] = '2';
        markOcean(r+1, c, R, C);
        markOcean(r-1, c, R, C);
        markOcean(r, c+1, R, C);
...
void countPerimeter(int r, int c, int R, int C) {
    if (val_bounds(r, c, R, C) && grid[r][c] == '1') {
        if (r-1 < 0 || grid[r-1][c] == '2') total++;
        if (r+1 >= R || grid[r+1][c] == '2') total++;
        if (c-1 < 0 || grid[r][c-1] == '2') total++;
        if (c+1 >= C || grid[r][c+1] == '2') total++;
        grid[r][c] = '3';

        countPerimeter(r+1, c, R, C);
        countPerimeter(r-1, c, R, C);
        countPerimeter(r, c+1, R, C);
        countPerimeter(r, c-1, R, C);
    }
}
...
int main() {
    ...
    // mark out ocean. Everything touching the border is the ocean
    for (int i = 0; i < R; i++)
        if (grid[i][0] == '0')
            markOcean(i, 0, R, C);
    for (int i = 0; i < R; i++)
