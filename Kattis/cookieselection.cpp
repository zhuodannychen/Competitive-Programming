    ...
    while (cin >> x) {
        if (x == "#") {
            if (lowerhalf.size() == upperhalf.size()) {
                int cur = upperhalf.top(); upperhalf.pop();
                cout << cur << endl;
            } else {
                int cur = lowerhalf.top(); lowerhalf.pop();
                cout << cur << endl;
            }
        } else {
            int val = stoi(x);
            ...
            if (val <= lowerhalf.top()) {
                lowerhalf.push(val);
                if (lowerhalf.size() > upperhalf.size()+1) {
                    int shift = lowerhalf.top(); lowerhalf.pop();
                    upperhalf.push(shift);
                }
            } else {
                upperhalf.push(val);
                if (lowerhalf.size()+1 <= upperhalf.size()) {
                    int shift = upperhalf.top(); upperhalf.pop();
                    lowerhalf.push(shift);
                }
            }
        }
    }
