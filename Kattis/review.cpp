...
int tech[100005], costs[100005], vis[100005];
ll ans[100005];
vi adj[100005];
...
void dfs(int node) {
    vis[node] = 1;
    ll prev = ft.sum(tech[node]-1);
    for (int child : adj[node]) {
        if (!vis[child]) {
            dfs(child);
        }
    }
    ans[node] = ft.sum(tech[node]-1) - prev;
    ft.update(tech[node], costs[node]);
}
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        int manager, techLvl, cost; cin >> manager >> techLvl >> cost;
        tech[i] = techLvl;
        costs[i] = cost;
        if (manager != -1) {
            adj[manager].pb(i);
