...
bool possible(int longest) {
    int cur = 0;
    int need = 0;
    f0r (i, W) {
        if (freights[i] <= cur) {
            // current freight belongs in same wagon
            continue;
        } else {
            if (freights[i] <= cur + longest) {
                // create new wagon to luxembourg immediately
                need++;
                cur += longest;
            } else {
                // send all the empty wagons before current freight to netherlands
                need += 2;
                cur = freights[i]-1 + longest;
            }
        }
    }
    // need to cover all the empty wagons after we sent all the freights
    if (cur < N) need++;
    return need <= L;
}
...
int main() {
    ...
    while (t--) {
        ...
        freights.resize(W);
        freights.clear();
        f0r (i, W) cin >> freights[i];
        int left = 1;
        int right = N;
        int res = 0;
        while (left <= right) {
            // min length of the longest train to Luxembourg
