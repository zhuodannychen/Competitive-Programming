...
vector<string> validGrid = {"11111", "01111", "00 11", "00001", "00000"};
...
void bfs(vector<string>& grid) {
    ...
    while (!q.empty()) {
        ...
        vector<string> newGrid = cur;
        for (int dr = 0; dr < 8; dr++) {
            int newx = x + dx[dr];
            int newy = y + dy[dr];
            if (newx < 0 || newx >= 5 || newy < 0 || newy >= 5)
                continue;
            swap(newGrid[x][y], newGrid[newx][newy]);
            if (dp.find(newGrid) == dp.end()) {
                dp[newGrid] = dp[cur] + 1;
                q.push(newGrid);
            }
            swap(newGrid[x][y], newGrid[newx][newy]);
...
int main() {
    ...
    while (t--) {
        ...
        if (dp.find(grid) != dp.end())
            cout << "Solvable in " << dp[grid] << " move(s).\n";
        else
