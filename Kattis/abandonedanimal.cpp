...
int main() {
    fastio;
    int n, k; cin >> n >> k;
    vector<set<string>> items(n);
    f0r (i, k) {
        int store; cin >> store;
        string item; cin >> item;
        items[store].insert(item);
    }

    int m; cin >> m;
    vector<string> bought(m);
    f0r (i, m) cin >> bought[i];

    vi forward;
    int curstore = 0;
    int itemNum = 0;
    bool forwardSol = true;
    while (itemNum < m) {
        if (items[curstore].count(bought[itemNum])) {
            forward.pb(curstore);
            itemNum++;
        } else {
            curstore++;
    ...
    if (forward == backward) cout << "unique\n";
    else cout << "ambiguous\n";
}
