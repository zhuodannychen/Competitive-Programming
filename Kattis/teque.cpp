    ...
    while (n--) {
        ...
        if (command == "push_back") {
            ...
            if (lower.size() < upper.size()) {
                int mid = upper.front(); upper.pop_front();
                lower.push_back(mid);
            }
        } else if (command == "push_front") {
            lower.push_front(val);
            if (lower.size() > upper.size()+1) {
                int mid = lower.back(); lower.pop_back();
                upper.push_front(mid);
            }
        } else if (command == "push_middle") {
            if (lower.size() == upper.size())
                lower.push_back(val);
            else
                upper.push_front(val);
        } else {
            if (val < lower.size())
                cout << lower[val] << endl;
            else 
                cout << upper[val-lower.size()] << endl;
        }
    }
}
