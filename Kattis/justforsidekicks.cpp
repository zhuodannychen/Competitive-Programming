...
int main() {
    ...
    // the idea is to keep a fenwick tree for each gem to account for the diff values.
    vector<FenwickTree> fts(7, FenwickTree(n));
    for (int i = 0; i < data.size(); i++)
        fts[data[i]-'0'].add(i, 1);
    //dbg(fts[5].sum(0, 7));
    while (q--) {
        int op, a, b; cin >> op >> a >> b;
        if (op == 1) {
            fts[data[a-1]-'0'].add(a-1, -1);
            data[a-1] = b+'0';
            fts[data[a-1]-'0'].add(a-1, 1);
        } else if (op == 2) {
            value[a-1] = b;
            ...
            ll ans = 0;
            f0r (i, 6) ans += value[i] * fts[i+1].sum(a-1, b-1);
            cout << ans << endl;
        }
        //dbg(value);
    }
