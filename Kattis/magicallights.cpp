...
vector<vi> adj;
int bit[300005][101], colors[300005], cnt, n;
pair<int, int> interval[300005];

int sum(int r, int color) {
    int ret = 0;
    for (; r >= 0; r = (r & (r + 1)) - 1)
        ret += bit[r][color];
    return ret;
...
int sum(int l, int r, int color) {
    return sum(r, color) - sum(l - 1, color);
}

void add(int idx, int color, int delta) {
    for (; idx < n; idx = idx | (idx + 1))
        bit[idx][color] += delta;
}

void dfs(int node) {
    interval[node].ff = cnt++;
    for (int child : adj[node])
        dfs(child);
    interval[node].ss = cnt-1;
}
...
int main() {
    ...
    f0r (i, n) cin >> colors[i];
    f0r (i, n-1) {
        int child; cin >> child;
        adj[child-1].pb(i+1);
    }

    // flatten tree to array for fenwick tree range queries
    dfs(0);
    f0r (i, n) {
        //debugm(interval[i].ff, interval[i].ss);
        add(interval[i].ff, colors[i], 1);
    }
    ...
    while (q--) {
        ...
        if (type == 0) {
            int ans = 0;
            f0r (i, 100) {
                int cnt = sum(interval[val].ff, interval[val].ss, i+1);
                if (cnt % 2 == 1)
                    ans++;
            }
            cout << ans << endl;
        } else {
            add(interval[val].ff, colors[val], -1);
            colors[val] = type;
            add(interval[val].ff, colors[val], 1);
        }
    }
