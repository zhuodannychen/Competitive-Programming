...
bool vis[10010];
int n, m, timer, ans, entry[10010], low[10010];
vector<vi> adj;
set<pii> bridges;
...
void findBridges(int node, int par) {
    vis[node] = true;
    entry[node] = low[node] = timer++;
    for (int child : adj[node]) {
        if (child == par) continue;
        if (vis[child]) low[node] = min(low[node], entry[child]);
        else {
...
void dfs(int node, int par) {
    ...
    ans++;
    for (int child : adj[node]) {
        if (bridges.count({child, node}) || bridges.count({node, child}))
            continue;
        if (!vis[child])
            dfs(child, node);
    }
