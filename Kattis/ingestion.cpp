...
int solve(int idx, int cap) {
    ...
    int best = solve(idx+1, 2*cap/3) + min(calories[idx], cap);
    best = max(best, solve(idx+2, cap) + min(calories[idx], cap));
    best = max(best, solve(idx+1, cap));
    best = max(best, solve(idx+2, m));
    //cout << idx << " " << cap << " "<< best << endl;
    return dp[idx][cap] = best;
}
