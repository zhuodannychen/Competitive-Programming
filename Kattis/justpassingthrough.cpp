...
int r, c, n, dp[501][501][11], grid[501][501];

bool validPass(int x, int y) {
    if (x == 0 || x == r-1 || y == 0 || y == c-1)
        return false;
    if (grid[x][y] == -1 || grid[x+1][y] == -1 || grid[x-1][y] == -1 || grid[x][y-1] == -1 || grid[x][y+1] == -1)
        return false;
    if (grid[x][y] > grid[x][y+1] && grid[x][y] > grid[x][y-1] && grid[x][y] < grid[x+1][y] && grid[x][y] < grid[x-1][y])
        return true;
    return false;
}
...
int solve(int x, int y, int cnt) {
    ...
    int best = inf;
    int cur = inf;
    if (x+1 < r && grid[x+1][y+1] != -1) {
        if (validPass(x+1, y+1))
            cur = solve(x+1, y+1, cnt+1) + grid[x+1][y+1];
        else
            cur = solve(x+1, y+1, cnt) + grid[x+1][y+1];
        best = min(cur, best);
    } 
    if (grid[x][y+1] != -1) {
        if (validPass(x, y+1))
            cur = solve(x, y+1, cnt+1) + grid[x][y+1];
        else
            cur = solve(x, y+1, cnt) + grid[x][y+1];
        best = min(cur, best);
    } 
...
int main() {
    ...
    for (int i = 0; i < r; i++) {
        if (grid[i][0] != -1)
            ans = min(ans, solve(i, 0, 0) + grid[i][0]);
    }
