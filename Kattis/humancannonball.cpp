...
int main() {
    ...
    // constructing graph with adj matrix
    f0r (i, n+2) 
        f0r (j, n+2) 
            adj[i][j] = (i == j ? 0 : inf);

    for (int i = 1; i < n+2; i++) 
        adj[0][i] = dist(locs[i].ff, locs[i].ss, locs[0].ff, locs[0].ss) / 5;
    for (int i = 1; i < n+1; i++) {
        for (int j = 1; j < n+2; j++) {
            double dis = dist(locs[i].ff, locs[i].ss, locs[j].ff, locs[j].ss);
            adj[i][j] = min(dis / 5, abs(dis - 50)/5 + 2);
        }
    }
