...
public:
    ...
    UnionFind(int N, vector<int> passengers) {
        setSize = passengers;
        numSets = 0; rank.assign(N, 0);
        p.assign(N, 0);
        for (int i = 0; i < N; i++)
            p[i] = i;
    }
...
int main() {
    ...
    UnionFind uf(100005, passengers);
    ...
    for (int i = n-1; i >= 0; i--) {
        vis[order[i]] = true;
        segments++;
        amt += round2ten(uf.sizeOfSet(order[i]));

        if (order[i]-1 >= 0) {
            if (vis[order[i]-1]) {
                amt -= round2ten(uf.sizeOfSet(order[i]));
                amt -= round2ten(uf.sizeOfSet(order[i]-1));
                uf.unionSet(order[i], order[i]-1);
                amt += round2ten(uf.sizeOfSet(order[i]));
                segments--;
            }
        }
