...
int main() {
    fastio;
    int n; cin >> n;
    map<string, int> nodesToInt;
    map<int, string> intToNodes;
    ...
    f0r(i, m) {
        string u, v; cin >> u >> v;
        adj[nodesToInt[u]].pb(nodesToInt[v]);
        adj[nodesToInt[v]].pb(nodesToInt[u]);
    }
        ...
        if (side[node] == -1) {
            ...
            while (!q.empty()) {
                ...
                for (int child : adj[parent]) {
                    if (side[child] == -1) {
                        side[child] = side[parent] ^ 1;
                        q.push(child);
                    } else {
                        isBipartite &= side[child] != side[parent];
                    }
                }
            }
        }
    ...
    if (!isBipartite)
        ...
        for (int i = 0; i < n; i++) if (side[i] == 0) cout << intToNodes[i] << " ";
        cout << endl;
        for (int i = 0; i < n; i++) if (side[i] == 1) cout << intToNodes[i] << " ";
    }
}
