...
// dp[row][k][side]
int grid[205][2], dp[205][205][2];
int n;
int closeCost = inf;

int solve(int row, int k, int side) {
    if (k == 0)
        return 0;
    if (row == n)
        return inf;
    if (dp[row][k][side] != -1)
        return dp[row][k][side];

    if (side == 0)
        return dp[row][k][side] = min(solve(row+1, k-1, 0) + grid[row][0], min(solve(row+1, k, 0), solve(row+1, k, 1)));
    else
        return dp[row][k][side] = min(solve(row+1, k-1, 1) + grid[row][1], min(solve(row+1, k, 0), solve(row+1, k, 1)));
}
...
int main() {
    ...
    cout << total - min(solve(0, k, 0), solve(0, k, 1)) << endl;
}
