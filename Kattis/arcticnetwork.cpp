...
int main() {
    ...
    while (t--) {
        ...
        vector<pii> locs(m);
        f0r (i, m) {
            cin >> locs[i].ff >> locs[i].ss;
        }

        vector<edge> edges;
        for (int i = 0; i < m; i++) {
            for (int j = i+1; j < m; j++) {
                int dist = (locs[i].ff-locs[j].ff)*(locs[i].ff-locs[j].ff) + (locs[i].ss-locs[j].ss)*(locs[i].ss-locs[j].ss);
        ...
        double ans = inf;
        for (edge e : edges) {
            //debugm(sqrt(e.weight), e.u, e.v);
            if (uf.numDisjointSets() == n)
                break;
