...
int main() {
    ...
    f0r (t, n) {
        ...
        if (op == "E") {
            ...
            int num; cin >> num;
            while (num--) {
                string event = stackEvents.top(); stackEvents.pop();
                events.erase(event);
                cnt--;
            }
        } else {
            ...
            vector<string> inp(scenarios);
            f0r(i, scenarios) cin >> inp[i];

            f0r (i, scenarios) {
                string scenario = inp[i];
                if (scenario[0] == '!') {
                    scenario = scenario.substr(1);
                    if (events.find(scenario) != events.end())
                        earliest = min(earliest, events[scenario]);
                } else {
                    if (events.find(scenario) == events.end()) {
                        cout << "Plot Error\n";
                        error = true;
                        ...
                        latest = max(latest, events[scenario]);
                    }
                }
            }
            ...
            if (!error) {
                ...
                if (earliest <= latest) {
                    cout << "Plot Error\n";
                } else if (earliest == cnt) {
                    cout << "Yes\n";
                } else {
                    cout << cnt - earliest << " Just A Dream\n";
                }
            }
        }
    }
