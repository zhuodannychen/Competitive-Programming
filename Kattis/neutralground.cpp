...
int n, m;
constexpr int nodes = 40*40*2 + 10;

int hashy(int i, int j, int l) {
    int base = (l == 1 ? n*m : 0);
    return (i*m + j) + base;
}
...
// Need 2*r*c nodes. Two nodes for each location. This represents that
// each cell has a capacity

int main() {
    fastio;
    cin >> m >> n;
    vector<pii> partyA, partyB;
    f0r (i, n) {
        f0r (j, m) {
            cin >> grid[i][j];
            if (grid[i][j] == 'A')
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (grid[i][j] != 'A' && grid[i][j] != 'B') {
                network.add(hashy(i, j, 0), hashy(i, j, 1), grid[i][j]-'0');
            } else {
                network.add(hashy(i, j, 0), hashy(i, j, 1), inf);
            }
        }
    }
        ...
        for (int j = 0; j < m; j++) {
            for (int dr = 0; dr < 4; dr++) {
                ...
                if (newx < 0 || newx >= n || newy < 0 || newy >= m)
                    continue;
                network.add(hashy(i, j, 1), hashy(newx, newy, 0), inf);
            }
        }
    }

    // connect partyB to sink
    for (auto b : partyB) {
        network.add(hashy(b.ff, b.ss, 1), SINK, inf);
    }

    // connect partyA to source
    for (auto a : partyA) {
        network.add(SOURCE, hashy(a.ff, a.ss, 0), inf);
    }
