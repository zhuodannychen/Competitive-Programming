...
int main() {
    ...
    pair<int, int> prev = rounds[0];
    if (prev.ff == 11 && prev.ss == 11) {
        cout << "error " << 1 << endl;
        return 0;
    }
    ...
    bool finished = false;
    for (int i = 1; i < n; i++) {
        //int curRound = rounds[i].ff + rounds[i].ss;
        //debug(prev);
        //debug(rounds[i]);
        if (rounds[i].ff == prev.ff && rounds[i].ss == prev.ss)
            continue;
        if (prev.ff == 11 || prev.ss == 11) {
            finished = true;
        }

        if (side(rounds[i].ff + rounds[i].ss) != side(prev.ff + prev.ss))
            swap(prev.ff, prev.ss);
        if (rounds[i].ff < prev.ff || rounds[i].ss < prev.ss) {
            cout << "error " << (i+1) << endl;
            return 0;
        }

        if (finished || (rounds[i].ff == 11 && rounds[i].ss == 11)) {
            cout << "error " << (i+1) << endl;
            return 0;
        }
