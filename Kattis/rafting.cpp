...
double pointLineDist(point pt, point a, point b) {
    int A = pt.x - a.x;
    int B = pt.y - a.y;
    int C = b.x - a.x;
    int D = b.y - a.y;
...
double lineLineDist(point a, point b, point c, point d) {
    double dist = inf;
    dist = min(dist, pointLineDist(a, c, d));
    dist = min(dist, pointLineDist(b, c, d));
    dist = min(dist, pointLineDist(c, a, b));
    dist = min(dist, pointLineDist(d, a, b));
    return dist;
}
    ...
    while (t--) {
        ...
        for (int i = 0; i < inner; i++) {
            for (int j = 0; j < outer; j++) {
                shortestDist = min(shortestDist, lineLineDist(innerPoints[i], innerPoints[(i+1)%inner], outerPoints[j], outerPoints[(j+1)%outer]));
            }
        }
        cout << fixed << setprecision(8) << shortestDist/2 << endl;
    }
}
